...
if __name__ == "__main__":
    ...
    no_match_tokens = pd.read_excel("chemical_matches2014.xlsx", sheet_name="no_matches")
        ...
        try:
            if len(names_containing_fragments[i]) > 0:
                if len(names_containing_fragments[i]) >= 2:
                    top_K_match = best_top_K_match(no_match_names[i], tokens[i], names_containing_fragments[i])
                    ratio_file.write(f"{no_match_names[i]} \t {tokens[i]} \t {top_K_match} \n")
                else:
                    ratio = fuzz.ratio(no_match_names[i], names_containing_fragments[i])
